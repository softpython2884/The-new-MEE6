...
function ModAssistantPageContent({ isPremium }: { isPremium: boolean }) {
    ...
    const handleActionChange = (severity: 'low' | 'medium' | 'high' | 'critical', action: string) => {
     ...
     const handleMultiSelectToggle = (type: 'exempt_roles' | 'exempt_channels', id: string) => {
        if (!config) return;
        const currentList = config[type] || [];
        const newList = currentList.includes(id)
            ? currentList.filter(currId => currId !== id)
            : [...currentList, id];
        handleValueChange(type, newList);
    };
                                    ...
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <Button variant="outline" className="w-full justify-between">
                                                <span className="truncate pr-2">{config.exempt_roles?.length || 0} rôle(s) sélectionné(s)</span>
                                        ...
                                        <DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width]">
                                            ...
                                            {roles.map(role => (
                                                <DropdownMenuCheckboxItem key={role.id} checked={config.exempt_roles?.includes(role.id)} onCheckedChange={() => handleMultiSelectToggle('exempt_roles', role.id)} onSelect={(e) => e.preventDefault()}>
                                                    {role.name}
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="exempt-channels" className="font-bold text-sm">Salons exemptés</Label>
                                     <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <Button variant="outline" className="w-full justify-between">
                                                 <span className="truncate pr-2">{config.exempt_channels?.length || 0} salon(s) sélectionné(s)</span>
                                                <ChevronDown className="ml-auto h-4 w-4" />
                                            </Button>
                                        </DropdownMenuTrigger>
                                        <DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width]">
                                            ...
                                            {channels.map(channel => (
                                                <DropdownMenuCheckboxItem key={channel.id} checked={config.exempt_channels?.includes(channel.id)} onCheckedChange={() => handleMultiSelectToggle('exempt_channels', channel.id)} onSelect={(e) => e.preventDefault()}>
                                                    # {channel.name}
                                                </DropdownMenuCheckboxItem>
                                            ))}
                                        </DropdownMenuContent>

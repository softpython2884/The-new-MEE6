...
const HelpCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        for (const command of commands.values()) {
            const commandPath = require.resolve(path.join(commandsBasePath, '..', '..', `commands/`)); // This is a bit of a hack to get the root commands dir path from inside the handler
            const commandDir = path.dirname(commandPath);
            const relativePath = path.relative(commandDir, commandPath);
            const category = relativePath.split(path.sep)[0];
            
            // A better way is to iterate over files and find which category the command belongs to.
